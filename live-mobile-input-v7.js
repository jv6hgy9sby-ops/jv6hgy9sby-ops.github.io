"use strict";
(function(){
  window.MATVEY_LIVE_BUILD="7.0-published-branch";

  var input=window.MatveyInput=window.MatveyInput||{
    moveX:0,
    moveY:0,
    run:false,
    action:false,
    actionPressed:false,
    source:"none"
  };

  var trackedObjects=[];
  var trackedScenes=[];
  var namedCandidates=[];
  var selectedPlayer=null;
  var selectedCamera=null;
  var selectedReason="searching";
  var lastSelectAt=0;
  var lastFrame=performance.now();
  var lastPosition=null;
  var movedDistance=0;
  var joystickPointerId=null;
  var runPointerId=null;
  var actionPointerId=null;
  var debugPanel=null;
  var originalEval=window.eval;

  function pushUnique(list,value){
    if(value&&list.indexOf(value)<0)list.push(value);
  }

  function clamp(value,min,max){
    return Math.max(min,Math.min(max,value));
  }

  function installThreeCapture(){
    if(!window.THREE||window.__matveyLiveThreeCapture)return;
    window.__matveyLiveThreeCapture=true;

    var originalAdd=THREE.Object3D.prototype.add;
    THREE.Object3D.prototype.add=function(){
      pushUnique(trackedObjects,this);
      for(var i=0;i<arguments.length;i++)pushUnique(trackedObjects,arguments[i]);
      return originalAdd.apply(this,arguments);
    };

    var OriginalScene=THREE.Scene;
    function LiveScene(){
      var scene=Reflect.construct(OriginalScene,arguments,new.target||LiveScene);
      pushUnique(trackedScenes,scene);
      pushUnique(trackedObjects,scene);
      return scene;
    }
    LiveScene.prototype=OriginalScene.prototype;
    Object.setPrototypeOf(LiveScene,OriginalScene);
    THREE.Scene=LiveScene;
  }

  function instrumentCore(source){
    if(typeof source!=="string")return source;
    var names={};
    var declaration=/\b(?:const|let|var)\s+([A-Za-z_$][\w$]*)/g;
    var match;
    while((match=declaration.exec(source))){
      if(/matvey|player|pug|dog|hero|character|avatar|camera|scene/i.test(match[1]))names[match[1]]=true;
    }

    var tail="\n;try{window.__matveyLiveNamed=window.__matveyLiveNamed||[];}catch(e){};\n";
    Object.keys(names).slice(0,120).forEach(function(name){
      tail+="try{window.__matveyLiveNamed.push({name:"+JSON.stringify(name)+",value:"+name+"});}catch(e){};\n";
    });
    return source+tail;
  }

  window.eval=function(source){
    if(typeof source==="string"&&source.indexOf("matvey-game-core")!==-1){
      source=instrumentCore(source);
    }
    return originalEval(source);
  };

  function objectFromValue(value){
    if(!value)return null;
    if(value.isObject3D)return value;
    var keys=["root","group","object","model","mesh","player","matvey","character","avatar","container"];
    for(var i=0;i<keys.length;i++){
      if(value[keys[i]]&&value[keys[i]].isObject3D)return value[keys[i]];
    }
    return null;
  }

  function collectNamed(){
    var list=window.__matveyLiveNamed||[];
    list.forEach(function(item){
      if(!item||!item.value)return;
      if(!namedCandidates.some(function(existing){return existing.name===item.name&&existing.value===item.value;})){
        namedCandidates.push(item);
      }
      var object=objectFromValue(item.value);
      if(object)pushUnique(trackedObjects,object);
      if(object&&object.isScene)pushUnique(trackedScenes,object);
    });
  }

  function descendants(object){
    var count=0;
    try{object.traverse(function(){count++;});}catch(error){}
    return count;
  }

  function subtreeText(object){
    var chunks=[];
    try{
      object.traverse(function(child){
        if(chunks.length>100)return;
        if(child.name)chunks.push(child.name);
        if(child.userData){
          try{chunks.push(JSON.stringify(child.userData));}catch(error){}
        }
      });
    }catch(error){}
    return chunks.join(" ").toLowerCase();
  }

  function boxInfo(object){
    if(!window.THREE||!object||!object.isObject3D)return null;
    try{
      object.updateWorldMatrix(true,true);
      var box=new THREE.Box3().setFromObject(object);
      if(box.isEmpty())return null;
      var size=new THREE.Vector3();
      var center=new THREE.Vector3();
      box.getSize(size);
      box.getCenter(center);
      return {box:box,size:size,center:center};
    }catch(error){return null;}
  }

  function findCamera(){
    collectNamed();
    for(var i=0;i<namedCandidates.length;i++){
      if(/camera/i.test(namedCandidates[i].name)){
        var candidate=objectFromValue(namedCandidates[i].value);
        if(candidate&&candidate.isCamera)return candidate;
      }
    }
    for(var j=0;j<trackedObjects.length;j++){
      if(trackedObjects[j]&&trackedObjects[j].isPerspectiveCamera)return trackedObjects[j];
    }
    for(var s=0;s<trackedScenes.length;s++){
      var found=null;
      try{trackedScenes[s].traverse(function(object){if(!found&&object.isPerspectiveCamera)found=object;});}catch(error){}
      if(found)return found;
    }
    return null;
  }

  function scoreCandidate(object,label,camera){
    if(!object||!object.isObject3D||object.isScene||object.isCamera||object.isLight)return -Infinity;
    if(object.visible===false)return -Infinity;

    var text=((label||"")+" "+(object.name||"")+" "+subtreeText(object)).toLowerCase();
    var score=0;
    if(/matvey|матвей/.test(text))score+=1500;
    if(/pug|мопс/.test(text))score+=1200;
    if(/player|hero|avatar/.test(text))score+=900;
    if(/dog|character/.test(text))score+=600;
    if(/head|body|leg|paw|tail|ear|muzzle|harness/.test(text))score+=220;
    if(/nastya|nikita|human|person|npc/.test(text))score-=900;
    if(/room|floor|wall|door|bed|sofa|table|chair|vacuum|crumb|smell|light|scene|world|house|ceiling/.test(text))score-=800;

    var info=boxInfo(object);
    if(!info)return score-700;
    var size=info.size;
    var max=Math.max(size.x,size.y,size.z);
    var min=Math.min(size.x,size.y,size.z);
    var count=descendants(object);

    if(max>.25&&max<3.5)score+=260;
    else if(max>=3.5)score-=700;
    if(size.y>.2&&size.y<2)score+=180;
    if(size.y>=2.5)score-=400;
    if(count>=5&&count<=220)score+=Math.min(300,count*4);
    if(count<3)score-=140;
    if(min===0)score-=80;
    if(object.parent&&object.parent.isScene)score+=120;

    if(camera){
      try{
        var projected=info.center.clone().project(camera);
        if(projected.z>-1&&projected.z<1){
          score+=Math.max(0,260-Math.hypot(projected.x,projected.y)*220);
        }
      }catch(error){}
    }
    return score;
  }

  function movableRoot(object){
    var current=object;
    while(current&&current.parent&&!current.parent.isScene){
      var parent=current.parent;
      var text=((parent.name||"")+" "+subtreeText(parent)).toLowerCase();
      var info=boxInfo(parent);
      var max=info?Math.max(info.size.x,info.size.y,info.size.z):Infinity;
      if(/room|world|house|environment|level|floor/.test(text))break;
      if(max>4.5||descendants(parent)>260)break;
      current=parent;
    }
    return current||object;
  }

  function gatherCandidates(){
    collectNamed();
    var result=[];
    namedCandidates.forEach(function(item){
      var object=objectFromValue(item.value);
      if(object)result.push({object:object,label:item.name});
    });
    trackedScenes.forEach(function(scene){
      try{scene.traverse(function(object){if(object&&object.isObject3D)result.push({object:object,label:object.name||"scene-object"});});}catch(error){}
    });
    trackedObjects.forEach(function(object){if(object&&object.isObject3D)result.push({object:object,label:object.name||"tracked-object"});});

    var seen=[];
    return result.filter(function(item){
      if(seen.indexOf(item.object)>=0)return false;
      seen.push(item.object);
      return true;
    });
  }

  function selectPlayer(force){
    var now=performance.now();
    if(!force&&selectedPlayer&&selectedPlayer.parent&&now-lastSelectAt<2500)return selectedPlayer;
    selectedCamera=findCamera()||selectedCamera;
    var best=null;
    var bestScore=-Infinity;
    gatherCandidates().forEach(function(item){
      var score=scoreCandidate(item.object,item.label,selectedCamera);
      if(score>bestScore){bestScore=score;best=item;}
    });
    if(best&&bestScore>100){
      selectedPlayer=movableRoot(best.object);
      selectedReason=(best.label||best.object.name||"object")+" score="+Math.round(bestScore);
      lastSelectAt=now;
      window.__matveyLivePlayer=selectedPlayer;
      window.__matveyLivePlayerScore=bestScore;
    }
    return selectedPlayer;
  }

  function gameAllowsMovement(){
    var hud=document.getElementById("hud");
    var start=document.getElementById("screen-start");
    var pause=document.getElementById("screen-pause");
    var finale=document.getElementById("screen-finale");
    if(!hud||hud.classList.contains("hidden"))return false;
    if(start&&!start.classList.contains("hidden"))return false;
    if(pause&&!pause.classList.contains("hidden"))return false;
    if(finale&&!finale.classList.contains("hidden"))return false;
    return true;
  }

  function movePlayer(dt){
    if(!gameAllowsMovement())return;
    var x=Number(input.moveX)||0;
    var y=Number(input.moveY)||0;
    var magnitude=Math.hypot(x,y);
    if(magnitude<.08)return;

    var player=selectPlayer(false);
    var camera=selectedCamera||findCamera();
    if(!player||!camera)return;

    var before=player.position.clone();
    var forward=new THREE.Vector3();
    camera.getWorldDirection(forward);
    forward.y=0;
    if(forward.lengthSq()<.0001)forward.set(0,0,-1);
    forward.normalize();
    var right=new THREE.Vector3(-forward.z,0,forward.x);
    var direction=forward.multiplyScalar(y).add(right.multiplyScalar(x));
    if(direction.lengthSq()<.0001)return;
    var analog=Math.min(1,direction.length());
    direction.normalize();

    var speed=(input.run?4.4:2.25)*analog;
    var distance=speed*Math.min(dt,.05);
    player.position.x+=direction.x*distance;
    player.position.z+=direction.z*distance;

    var targetYaw=Math.atan2(direction.x,direction.z);
    var diff=Math.atan2(Math.sin(targetYaw-player.rotation.y),Math.cos(targetYaw-player.rotation.y));
    player.rotation.y+=diff*Math.min(1,dt*11);
    player.updateMatrixWorld(true);

    movedDistance+=before.distanceTo(player.position);
    lastPosition=player.position.clone();
  }

  function resetInput(){
    input.moveX=0;
    input.moveY=0;
    input.run=false;
    input.action=false;
    input.actionPressed=false;
    joystickPointerId=null;
    runPointerId=null;
    actionPointerId=null;
    var knob=document.getElementById("joystick-knob");
    if(knob)knob.style.transform="translate(0px, 0px)";
    var run=document.getElementById("btn-run");
    var action=document.getElementById("btn-action");
    if(run)run.classList.remove("active");
    if(action)action.classList.remove("active");
  }

  function installTouchControls(){
    var zone=document.getElementById("joystick-zone");
    var base=document.getElementById("joystick-base");
    var knob=document.getElementById("joystick-knob");
    var run=document.getElementById("btn-run");
    var action=document.getElementById("btn-action");
    if(!zone||!base||!knob||!run||!action)return;

    zone.style.touchAction="none";
    run.style.touchAction="none";
    action.style.touchAction="none";

    function update(clientX,clientY){
      var rect=base.getBoundingClientRect();
      var cx=rect.left+rect.width/2;
      var cy=rect.top+rect.height/2;
      var dx=clientX-cx;
      var dy=clientY-cy;
      var radius=Math.max(28,rect.width*.34);
      var length=Math.hypot(dx,dy)||1;
      if(length>radius){dx=dx/length*radius;dy=dy/length*radius;}
      var x=clamp(dx/radius,-1,1);
      var y=clamp(-dy/radius,-1,1);
      var magnitude=Math.hypot(x,y);
      if(magnitude<.14){x=0;y=0;}
      else if(magnitude>1){x/=magnitude;y/=magnitude;}
      input.moveX=x;
      input.moveY=y;
      input.source="touch-v7";
      knob.style.transform="translate("+dx.toFixed(1)+"px,"+dy.toFixed(1)+"px)";
    }

    function endJoystick(event){
      if(joystickPointerId!==null&&event&&event.pointerId!==joystickPointerId)return;
      joystickPointerId=null;
      input.moveX=0;
      input.moveY=0;
      knob.style.transform="translate(0px, 0px)";
    }

    if(window.PointerEvent){
      zone.addEventListener("pointerdown",function(event){
        if(event.pointerType==="mouse"&&event.button!==0)return;
        event.preventDefault();
        event.stopPropagation();
        joystickPointerId=event.pointerId;
        try{zone.setPointerCapture(event.pointerId);}catch(error){}
        update(event.clientX,event.clientY);
      },{passive:false,capture:true});
      zone.addEventListener("pointermove",function(event){
        if(event.pointerId!==joystickPointerId)return;
        event.preventDefault();
        event.stopPropagation();
        update(event.clientX,event.clientY);
      },{passive:false,capture:true});
      zone.addEventListener("pointerup",endJoystick,{passive:false,capture:true});
      zone.addEventListener("pointercancel",endJoystick,{passive:false,capture:true});
      zone.addEventListener("lostpointercapture",endJoystick,true);

      run.addEventListener("pointerdown",function(event){
        event.preventDefault();
        event.stopPropagation();
        runPointerId=event.pointerId;
        input.run=true;
        input.source="touch-v7";
        run.classList.add("active");
        try{run.setPointerCapture(event.pointerId);}catch(error){}
      },{passive:false,capture:true});
      function endRun(event){
        if(runPointerId!==null&&event&&event.pointerId!==runPointerId)return;
        runPointerId=null;
        input.run=false;
        run.classList.remove("active");
      }
      run.addEventListener("pointerup",endRun,{passive:false,capture:true});
      run.addEventListener("pointercancel",endRun,{passive:false,capture:true});
      run.addEventListener("lostpointercapture",endRun,true);

      action.addEventListener("pointerdown",function(event){
        event.preventDefault();
        event.stopPropagation();
        actionPointerId=event.pointerId;
        input.action=true;
        input.actionPressed=true;
        input.source="touch-v7";
        action.classList.add("active");
        try{action.setPointerCapture(event.pointerId);}catch(error){}
        requestAnimationFrame(function(){input.actionPressed=false;});
      },{passive:false,capture:true});
      function endAction(event){
        if(actionPointerId!==null&&event&&event.pointerId!==actionPointerId)return;
        actionPointerId=null;
        input.action=false;
        input.actionPressed=false;
        action.classList.remove("active");
      }
      action.addEventListener("pointerup",endAction,{passive:false,capture:true});
      action.addEventListener("pointercancel",endAction,{passive:false,capture:true});
      action.addEventListener("lostpointercapture",endAction,true);
    }

    window.addEventListener("blur",resetInput);
    window.addEventListener("pagehide",resetInput);
    window.addEventListener("orientationchange",resetInput);
    document.addEventListener("visibilitychange",function(){if(document.hidden)resetInput();});
  }

  function addBuildBadge(){
    var badge=document.createElement("div");
    badge.id="matvey-live-build-badge";
    badge.textContent="LIVE BUILD 7.0";
    badge.style.cssText="position:fixed;left:8px;top:max(8px,env(safe-area-inset-top));z-index:10050;background:#102515;color:#b9ffbd;border:1px solid #58d866;border-radius:8px;padding:5px 8px;font:700 10px/1.2 monospace;pointer-events:none;box-shadow:0 4px 18px rgba(0,0,0,.35)";
    document.body.appendChild(badge);
  }

  function installDebug(){
    var params=new URLSearchParams(location.search);
    if(!params.has("debug")&&!params.has("nativefix")&&!params.has("inputdebug"))return;
    debugPanel=document.createElement("div");
    debugPanel.style.cssText="position:fixed;right:8px;bottom:max(8px,env(safe-area-inset-bottom));z-index:10050;max-width:56vw;background:rgba(5,8,5,.9);color:#caffb7;border:1px solid rgba(168,255,137,.55);border-radius:10px;padding:7px 9px;font:10px/1.32 monospace;pointer-events:none;white-space:pre-wrap";
    document.body.appendChild(debugPanel);
    setInterval(function(){
      var player=selectedPlayer;
      var pos=player?player.position:null;
      debugPanel.textContent=
        "LIVE 7.0 | loader "+String(window.MATVEY_LOADER_BUILD||"unknown")+"\n"+
        "input "+Number(input.moveX||0).toFixed(2)+","+Number(input.moveY||0).toFixed(2)+" run="+Boolean(input.run)+"\n"+
        "player "+selectedReason+"\n"+
        "pos "+(pos?[pos.x.toFixed(2),pos.y.toFixed(2),pos.z.toFixed(2)].join(","):"none")+" moved="+movedDistance.toFixed(2)+"\n"+
        "scenes="+trackedScenes.length+" objects="+trackedObjects.length+" named="+namedCandidates.length;
    },200);
  }

  function loop(now){
    var dt=(now-lastFrame)/1000;
    lastFrame=now;
    try{movePlayer(dt);}catch(error){console.error("Matvey live movement v7:",error);}
    requestAnimationFrame(loop);
  }

  function boot(){
    installThreeCapture();
    addBuildBadge();
    installTouchControls();
    installDebug();
    setInterval(function(){selectPlayer(false);},750);
    setTimeout(function(){lastFrame=performance.now();requestAnimationFrame(loop);},1400);
  }

  installThreeCapture();
  if(document.readyState==="loading")document.addEventListener("DOMContentLoaded",boot,{once:true});
  else boot();
})();
