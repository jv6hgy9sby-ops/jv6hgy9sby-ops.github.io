...
var settings={sound:true,music:.18,voice:.9,sfx:.7,sens:1,calm:false,quality:'medium'};
...
function chooseRussianVoice(){
  ...
  var strictMale=/\b(yuri|yury|юрий|pavel|павел|maxim|maksim|максим|alexander|aleksandr|александр|nikolai|николай|mikhail|михаил|dmitry|дмитрий)\b/i;
  return voices.find(function(v){return(/^ru/i).test(v.lang||'')&&strictMale.test(v.name||'')})||null;
}
...
function stopVoice(){
  ...
  if(voiceState.audio){try{voiceState.audio.pause();voiceState.audio.currentTime=0}catch(error){}voiceState.audio=null}
...
function speakMatvey(key,text,options){
  ...
  if(voiceState.bubbleTimer)clearTimeout(voiceState.bubbleTimer);
  if(voiceState.audio){try{voiceState.audio.pause()}catch(error){}voiceState.audio=null}
  ...
  if(!settings.sound||settings.voice<=0||!AudioManager.unlocked)return;
  ...
  AudioManager.probe(path).then(function(ok){
    ...
    var finish=function(){if(voiceState.audio===a)voiceState.audio=null;voiceState.speaking=false;AudioManager.duck(false)};
    a.onended=finish;
    a.onerror=finish;
    var p=a.play();if(p&&p.catch)p.catch(finish);
  });
}
...
renderer.domElement.addEventListener('webglcontextlost',function(event){event.preventDefault();if(Game&&Game.mode==='playing')pauseGame();window.__fatal('3D-контекст был потерян. Обновите страницу — прогресс достижений сохранён.')},false);
scene=new THREE.Scene();scene.background=new THREE.Color(0x304b4b);
camera=new THREE.PerspectiveCamera(49,1,.1,110);
var hemi=new THREE.HemisphereLight(0xffdfbd,0x263c37,.38);scene.add(hemi);
dirLight=new THREE.DirectionalLight(0xffc98c,1.12);dirLight.position.set(9,14,-8);dirLight.castShadow=true;dirLight.shadow.mapSize.set(1024,1024);dirLight.shadow.camera.left=-18;dirLight.shadow.camera.right=18;dirLight.shadow.camera.top=18;dirLight.shadow.camera.bottom=-18;dirLight.shadow.camera.near=2;dirLight.shadow.camera.far=44;dirLight.shadow.bias=-.0014;scene.add(dirLight);
rimLight=new THREE.DirectionalLight(0x8fc3c2,.24);rimLight.position.set(-10,8,8);scene.add(rimLight);
roomLight=new THREE.PointLight(0xffa95c,.32,18,2);roomLight.position.set(-2.5,3.8,1.5);scene.add(roomLight);

function canvasTexture(size,draw,repeatX,repeatY){
  var canvas=document.createElement('canvas');canvas.width=canvas.height=size;
  var ctx=canvas.getContext('2d');draw(ctx,size);
  var texture=new THREE.CanvasTexture(canvas);texture.encoding=THREE.sRGBEncoding;
  if(repeatX||repeatY){texture.wrapS=texture.wrapT=THREE.RepeatWrapping;texture.repeat.set(repeatX||1,repeatY||1)}
  texture.anisotropy=Math.min(4,renderer.capabilities.getMaxAnisotropy?renderer.capabilities.getMaxAnisotropy():1);
  return texture;
}
function makeWoodTexture(){
  return canvasTexture(512,function(ctx,s){
    ctx.fillStyle='#8b5735';ctx.fillRect(0,0,s,s);
    var board=s/4;
    for(var y=0;y<s;y+=board){
      var g=ctx.createLinearGradient(0,y,0,y+board);g.addColorStop(0,'#a66d43');g.addColorStop(.55,'#895331');g.addColorStop(1,'#754328');ctx.fillStyle=g;ctx.fillRect(0,y,s,board-3);
      ctx.fillStyle='rgba(42,22,12,.42)';ctx.fillRect(0,y+board-3,s,3);
      for(var i=0;i<85;i++){
        var px=Math.random()*s,py=y+Math.random()*(board-5),len=12+Math.random()*55;
        ctx.strokeStyle='rgba(66,32,16,'+(0.035+Math.random()*.07)+')';ctx.lineWidth=.5+Math.random();ctx.beginPath();ctx.moveTo(px,py);ctx.bezierCurveTo(px+len*.25,py+Math.sin(px)*3,px+len*.7,py-2,px+len,py+Math.random()*3);ctx.stroke();
      }
    }
  },3.2,4.4);
}
function makeTileTexture(){
  return canvasTexture(384,function(ctx,s){
    ctx.fillStyle='#6f716c';ctx.fillRect(0,0,s,s);var cell=s/4;
    for(var y=0;y<4;y++)for(var x=0;x<4;x++){
      var v=104+Math.floor(Math.random()*14);ctx.fillStyle='rgb('+v+','+(v+3)+','+(v+1)+')';ctx.fillRect(x*cell+2,y*cell+2,cell-4,cell-4);
      ctx.strokeStyle='rgba(255,238,211,.13)';ctx.strokeRect(x*cell+2.5,y*cell+2.5,cell-5,cell-5);
    }
  },2.5,4.6);
}
function makeFurTexture(){
  return canvasTexture(256,function(ctx,s){
    ctx.fillStyle='#bca079';ctx.fillRect(0,0,s,s);
    var image=ctx.getImageData(0,0,s,s),d=image.data;
    for(var i=0;i<d.length;i+=4){var n=(Math.random()-.5)*34;d[i]=clamp(d[i]+n,0,255);d[i+1]=clamp(d[i+1]+n*.88,0,255);d[i+2]=clamp(d[i+2]+n*.66,0,255)}
    ctx.putImageData(image,0,0);
    for(var k=0;k<430;k++){
      var x=Math.random()*s,y=Math.random()*s,l=2+Math.random()*7;
      ctx.strokeStyle='rgba(55,39,28,'+(0.025+Math.random()*.045)+')';ctx.lineWidth=.45;ctx.beginPath();ctx.moveTo(x,y);ctx.lineTo(x+Math.sin(y*.11)*1.5,y+l);ctx.stroke();
    }
  },2.2,2.2);
}
function makeFabricTexture(base){
  return canvasTexture(256,function(ctx,s){
    ...
    for(var i=0;i<s;i+=5){ctx.beginPath();ctx.moveTo(i,0);ctx.lineTo(i,s);ctx.stroke();ctx.beginPath();ctx.moveTo(0,i);ctx.lineTo(s,i);ctx.stroke()}
  },3,3);
}
var TEX={wood:makeWoodTexture(),tile:makeTileTexture(),fur:makeFurTexture(),sofa:makeFabricTexture('#23494a'),sheet:makeFabricTexture('#8d3040')};
function mat(color,rough,metal,map){
  var material=new THREE.MeshStandardMaterial({color:color,roughness:rough===undefined?.86:rough,metalness:metal||0,map:map||null});
  return material;
}
var M={
  wall:mat(0x9f8d76,.94),trim:mat(0x60452f,.88),wood:mat(0x80502f,.8),woodDark:mat(0x3f291f,.86),
  floor:mat(0xffffff,.78,0,TEX.wood),kitchenFloor:mat(0xffffff,.9,0,TEX.tile),grass:mat(0x3f6b42,.96),grass2:mat(0x527d45,.96),earth:mat(0x654832,.99),
  sofa:mat(0xffffff,.91,0,TEX.sofa),sofaDark:mat(0x153638,.96),gold:mat(0xc38a3b,.84),red:mat(0x7d2838,.9),red2:mat(0xa54755,.9),
  blue:mat(0x315f91,.72),blueDark:mat(0x173a62,.76),cream:mat(0xcab89e,.92),sheet:mat(0xffffff,.92,0,TEX.sheet),
  metal:mat(0x747e82,.32,.52),fridge:mat(0x879296,.34,.35),black:mat(0x121212,.42),robot:mat(0x20262b,.42,.32),
  pug:mat(0xc0a27b,.91,0,TEX.fur),pugLight:mat(0xd2b991,.93,0,TEX.fur),pugShade:mat(0x8f6c4e,.92,0,TEX.fur),pugDark:mat(0x29211e,.78),pugGrey:mat(0x6f665f,.88),
  eye:new THREE.MeshPhysicalMaterial({color:0x2a160f,roughness:.12,metalness:0,clearcoat:1,clearcoatRoughness:.08}),
  nose:new THREE.MeshPhysicalMaterial({color:0x090909,roughness:.18,metalness:0,clearcoat:.82,clearcoatRoughness:.13}),
  tongue:mat(0xc96879,.68),skin:mat(0xb98666,.9),pants:mat(0x34394a,.92),sweater:mat(0x356d69,.91),
  leaf:mat(0x335f35,.94),leaf2:mat(0x638a45,.94),path:mat(0x938673,.95),crumb:mat(0xf1b844,.7),nail:mat(0x302720,.65)
};
...
function wallX(x,z0,z1){
  ...
  box(.34,.055,Math.abs(z1-z0),M.trim,x,2.535,(z0+z1)/2,null,false,false);
...
function wallZ(z,x0,x1){
  ...
  box(Math.abs(x1-x0),.055,.34,M.trim,(x0+x1)/2,2.535,z,null,false,false);
...
function makeFloor(){
  ...
  for(var k=0;k<34;k++){
    var patch=mesh(new THREE.CircleGeometry(rand(.07,.31),8),Math.random()>.36?M.grass:M.earth,rand(1.8,10.2),.012,rand(-13.7,-7.2));
    patch.rotation.x=-Math.PI/2;patch.rotation.z=rand(0,Math.PI);patch.receiveShadow=true;
  }
  for(var t=0;t<18;t++){
    var tuft=new THREE.Group();tuft.position.set(rand(1.8,10.2),.015,rand(-13.7,-7.2));scene.add(tuft);
    for(var b=0;b<3;b++){var blade=mesh(new THREE.ConeGeometry(.025,.18+Math.random()*.12,4),Math.random()>.45?M.leaf:M.leaf2,rand(-.06,.06),.08,rand(-.06,.06),tuft);blade.rotation.z=rand(-.25,.25)}
  }
...
var bowl=cylinder(.18,.13,.09,M.red,-4.05,.055,6.28,null,14);var water=cylinder(.15,.11,.075,M.metal,-3.67,.048,6.28,null,14);
...
var tvScreen=mesh(new THREE.PlaneGeometry(1.48,.82),new THREE.MeshBasicMaterial({color:0x2f5a5e}),.6,1.34,-6.72);
...
/* Hall */
var hallRug=box(1.35,.025,.82,M.red2,5.8,.018,-6.08);[[5,-6.34,.25],[5.34,-6.4,-.25],[5.7,-6.3,.55]].forEach(function(s){var shoe=box(.13,.1,.31,M.woodDark,s[0],.05,s[1]);shoe.rotation.y=s[2]});
...
var sheet=box(2.34,.055,2.92,M.sheet,bedCX,.63,bedCZ,null,false,true);
...
var bedBase=sphere(.72,.14,.52,M.blue,0,.13,0,dogBed,16);var bedInner=sphere(.56,.1,.39,M.blueDark,0,.22,0,dogBed,16);addCollider(1.78,3.22,4.98,6.02);
var ball=sphere(.09,.09,.09,M.red,-1.9,.09,-.7,null,12);
...
(function addInteriorDetails(){
  function frame(x,y,z,w,h,color,wall){
    ...
    var border=box(w,h,.045,M.woodDark,0,0,0,g,true,false);
    var art=mesh(new THREE.PlaneGeometry(w-.12,h-.12),new THREE.MeshBasicMaterial({color:color}),0,0,wall==='z'?.028:.028,g);art.position.z=.028;
    return g;
  }
  var art1=frame(-2.98,1.65,3.45,.78,.62,0x315f62);art1.rotation.y=Math.PI/2;
  var art2=frame(4.02,1.7,-3.55,.64,.82,0x9d4f45);art2.rotation.y=-Math.PI/2;
  box(1.35,.055,.28,M.woodDark,-.4,1.25,-6.62,null,true,false);
  for(var i=0;i<6;i++)box(.11+Math.random()*.05,.34+Math.random()*.14,.22,[M.red,M.blue,M.gold,M.cream][i%4],-1+i*.25,1.45,-6.62,null,true,false);
  var lampBase=cylinder(.18,.22,.12,M.metal,2.65,.07,-5.75,null,16);lampBase.castShadow=true;
  cylinder(.035,.045,1.08,M.metal,2.65,.64,-5.75,null,10);
  var shade=new THREE.Mesh(new THREE.ConeGeometry(.32,.38,18,1,true),M.gold);shade.position.set(2.65,1.25,-5.75);shade.rotation.x=Math.PI;shade.castShadow=true;scene.add(shade);
  var lampGlow=new THREE.PointLight(0xffb05c,.18,5,2);lampGlow.position.set(2.65,1.18,-5.75);scene.add(lampGlow);
  for(var c=0;c<5;c++)box(.17,.06,.17,M.metal,-9.38,1.0,1.5+c*.66,null,true,false);
  var curtainMat=mat(0x8b3b48,.96);
  [-1.4,1.6].forEach(function(x){
    box(.08,1.42,.04,curtainMat,x-.96,1.52,6.72,null,false,false);box(.08,1.42,.04,curtainMat,x+.96,1.52,6.72,null,false,false);
  });
  var toy=new THREE.Group();toy.position.set(-1.65,.09,-.35);scene.add(toy);
  cylinder(.055,.055,.42,M.red,0,0,0,toy,10).rotation.z=Math.PI/2;
  sphere(.09,.09,.09,M.gold,-.22,0,0,toy,10);sphere(.09,.09,.09,M.gold,.22,0,0,toy,10);
  var foodMat=mat(0x9b5b2c,.82);
  for(var f=0;f<7;f++)sphere(.025,.018,.025,foodMat,-4.05+rand(-.1,.1),.105,6.28+rand(-.1,.1),null,7);
})();
...
(function buildPug(){
  ...
  P.body=sphere(.43,.305,.46,M.pug,0,.32,-.035,proceduralPug,24);
  P.chest=sphere(.39,.34,.29,M.pugLight,0,.34,.235,proceduralPug,22);
  P.rear=sphere(.405,.32,.32,M.pug,0,.33,-.30,proceduralPug,22);
  P.belly=sphere(.34,.205,.37,M.pugLight,0,.18,-.015,proceduralPug,20);
  P.shoulderL=sphere(.19,.245,.23,M.pugLight,.205,.31,.18,proceduralPug,18);
  P.shoulderR=sphere(.19,.245,.23,M.pugLight,-.205,.31,.18,proceduralPug,18);
  P.haunchL=sphere(.205,.25,.245,M.pug,.205,.30,-.27,proceduralPug,18);
  P.haunchR=sphere(.205,.25,.245,M.pug,-.205,.30,-.27,proceduralPug,18);
  P.stripe=sphere(.105,.038,.39,M.pugShade,0,.565,-.08,proceduralPug,16);P.stripe.rotation.x=.07;
  P.backFold=new THREE.Mesh(new THREE.TorusGeometry(.27,.026,8,26,Math.PI*1.45),M.pugShade);P.backFold.position.set(0,.53,-.24);P.backFold.rotation.set(Math.PI/2,.18,.78);proceduralPug.add(P.backFold);
  P.neckFold1=new THREE.Mesh(new THREE.TorusGeometry(.245,.046,10,28),M.pugShade);P.neckFold1.position.set(0,.49,.18);P.neckFold1.rotation.x=Math.PI/2-.25;proceduralPug.add(P.neckFold1);
  P.neckFold2=new THREE.Mesh(new THREE.TorusGeometry(.224,.034,9,26),M.pugLight);P.neckFold2.position.set(0,.525,.225);P.neckFold2.rotation.x=Math.PI/2-.25;proceduralPug.add(P.neckFold2);
  P.dewlap=sphere(.22,.115,.12,M.pugGrey,0,.41,.34,proceduralPug,18);

  P.head=new THREE.Group();P.head.position.set(0,.625,.37);proceduralPug.add(P.head);
  P.skull=sphere(.265,.235,.205,M.pug,0,0,0,P.head,26);
  P.forehead=sphere(.205,.15,.125,M.pugLight,0,.065,.075,P.head,22);
  P.foreheadStripe=sphere(.06,.135,.04,M.pugShade,.006,.075,.17,P.head,16);
  P.cheeks=[];
  [-1,1].forEach(function(side){P.cheeks.push(sphere(.125,.105,.105,M.pug,side*.12,-.055,.075,P.head,18))});
  P.mask=sphere(.185,.145,.12,M.pugDark,0,-.045,.12,P.head,20);
  P.eyeRings=[];P.eyes=[];P.brows=[];
  [-1,1].forEach(function(side,index){
    var ring=sphere(.082,.086,.06,M.pugDark,side*(index===0?.117:.113),.025,.147,P.head,20);P.eyeRings.push(ring);
    var e=sphere(index===0?.068:.065,index===0?.073:.069,.061,M.eye,side*(index===0?.119:.115),.025,.175,P.head,20);
    sphere(.019,.02,.008,new THREE.MeshBasicMaterial({color:0xffffff}),side*.014,.021,.058,e,9).castShadow=false;
    sphere(.008,.008,.004,new THREE.MeshBasicMaterial({color:0xd8f2ff}),-side*.018,-.016,.06,e,7).castShadow=false;
    P.eyes.push(e);
    var brow=sphere(.064,.021,.034,M.pugShade,side*.112,.125,.145,P.head,14);brow.rotation.z=-side*(index===0?.26:.18);P.brows.push(brow);
  });
  P.noseBridge=sphere(.07,.075,.052,M.pugDark,0,.035,.178,P.head,16);
  P.muzzles=[];
  [-1,1].forEach(function(side){P.muzzles.push(sphere(.087,.068,.08,M.pugDark,side*.061,-.09,.178,P.head,16))});
  P.muzzleGreyL=sphere(.052,.025,.055,M.pugGrey,-.051,-.108,.218,P.head,13);
  P.muzzleGreyR=sphere(.05,.023,.052,M.pugGrey,.052,-.11,.216,P.head,13);
  P.chin=sphere(.104,.052,.075,M.pugGrey,0,-.15,.145,P.head,16);
  P.jaw=new THREE.Group();P.jaw.position.set(0,-.132,.175);P.head.add(P.jaw);
  P.lowerLip=sphere(.082,.024,.048,M.pugDark,0,0,0,P.jaw,14);
  P.nose=sphere(.056,.043,.041,M.nose,0,-.048,.235,P.head,16);
  P.nostrilL=sphere(.012,.009,.008,M.black,-.021,-.046,.27,P.head,8);P.nostrilR=sphere(.012,.009,.008,M.black,.021,-.046,.27,P.head,8);
  P.ears=[];
  [-1,1].forEach(function(side){
    var ear=sphere(.075,.105,.047,M.pugDark,side*.195,.135,-.022,P.head,16);ear.rotation.set(.52,side*.06,-side*.5);P.ears.push(ear);
  });
  for(var f=0;f<4;f++){
    var fold=new THREE.Mesh(new THREE.TorusGeometry(.112-f*.021,.009,7,18,Math.PI*.86),M.pugShade);
    fold.position.set(f===0?.008:0,.155-f*.031,.132);fold.rotation.x=-.53;fold.rotation.z=.22+(f%2)*.035;P.head.add(fold);
  }
  P.tongue=sphere(.037,.015,.057,M.tongue,0,-.154,.205,P.head,11);P.tongue.visible=false;
  P.whiskerDots=[];
  [-1,1].forEach(function(side){for(var w=0;w<3;w++){var dot=sphere(.006,.005,.004,M.black,side*(.052+w*.014),-.095-w*.014,.248,P.head,6);dot.castShadow=false;P.whiskerDots.push(dot)}});

  function leg(x,z,front){
    var g=new THREE.Group();g.position.set(x,.30,z);proceduralPug.add(g);
    cylinder(.073,.082,front?.195:.185,M.pug,0,-.09,0,g,13);
    sphere(.087,.058,.108,M.pugShade,0,-.205,.033,g,14);
    for(var toe=-1;toe<=1;toe++){
      sphere(.027,.022,.038,M.pugShade,toe*.036,-.232,.09,g,9);
      var nail=sphere(.009,.009,.016,M.nail,toe*.036,-.238,.126,g,7);nail.castShadow=false;
    }
    return g;
  }
  P.legFL=leg(.19,.225,true);P.legFR=leg(-.19,.225,true);P.legRL=leg(.205,-.29,false);P.legRR=leg(-.205,-.29,false);
  P.tailGroup=new THREE.Group();P.tailGroup.position.set(0,.535,-.49);proceduralPug.add(P.tailGroup);
  P.tailOuter=new THREE.Mesh(new THREE.TorusGeometry(.086,.043,10,22,Math.PI*1.88),M.pug);P.tailOuter.rotation.x=Math.PI/2;P.tailOuter.rotation.z=.42;P.tailGroup.add(P.tailOuter);
  P.tailInner=new THREE.Mesh(new THREE.TorusGeometry(.043,.023,9,18,Math.PI*1.65),M.pugLight);P.tailInner.rotation.x=Math.PI/2;P.tailInner.rotation.z=-.32;P.tailInner.position.set(.018,.025,.012);P.tailGroup.add(P.tailInner);

  P.harness=new THREE.Group();proceduralPug.add(P.harness);
  var neck=new THREE.Mesh(new THREE.TorusGeometry(.235,.022,9,28),M.blue);neck.position.set(0,.505,.225);neck.rotation.x=Math.PI/2-.25;P.harness.add(neck);
  var chestRing=new THREE.Mesh(new THREE.TorusGeometry(.34,.027,9,28),M.blue);chestRing.position.set(0,.345,.035);chestRing.scale.z=.79;chestRing.rotation.x=Math.PI/2;P.harness.add(chestRing);
  box(.058,.315,.045,M.blue,0,.325,.34,P.harness,true,false);
  box(.068,.048,.19,M.blueDark,0,.58,.015,P.harness,true,false);
  box(.075,.052,.028,M.metal,0,.34,.37,P.harness,true,false);
  P.leashCarry=new THREE.Group();P.leashCarry.position.set(0,-.205,.30);P.head.add(P.leashCarry);
...
var shadowCanvas=document.createElement('canvas');shadowCanvas.width=shadowCanvas.height=128;var shadowCtx=shadowCanvas.getContext('2d');var shadowGrad=shadowCtx.createRadialGradient(64,64,8,64,64,60);shadowGrad.addColorStop(0,'rgba(0,0,0,.48)');shadowGrad.addColorStop(.55,'rgba(0,0,0,.22)');shadowGrad.addColorStop(1,'rgba(0,0,0,0)');shadowCtx.fillStyle=shadowGrad;shadowCtx.fillRect(0,0,128,128);var shadowTex=new THREE.CanvasTexture(shadowCanvas);
var pugShadow=mesh(new THREE.PlaneGeometry(1.15,.9),new THREE.MeshBasicMaterial({map:shadowTex,transparent:true,depthWrite:false,opacity:.48}),0,.012,-.02,pugRoot);pugShadow.rotation.x=-Math.PI/2;pugShadow.renderOrder=1;
var pugBase={
  bodyScale:P.body.scale.clone(),chestScale:P.chest.scale.clone(),rearScale:P.rear.scale.clone(),bellyScale:P.belly.scale.clone(),noseScale:P.nose.scale.clone(),
  eyeScales:P.eyes.map(function(e){return e.scale.clone()}),bodyPos:P.body.position.clone(),chestPos:P.chest.position.clone(),rearPos:P.rear.position.clone(),bellyPos:P.belly.position.clone(),
  shoulderL:P.shoulderL.position.clone(),shoulderR:P.shoulderR.position.clone(),haunchL:P.haunchL.position.clone(),haunchR:P.haunchR.position.clone()
};
...
function tryLoadGlb(){
  ...
  try{
    new THREE.GLTFLoader().load('assets/matvey.glb',function(gltf){
      ...
      var actions={},mixer=new THREE.AnimationMixer(gltf.scene);
      ...
      var scale=.7/size.y;gltf.scene.scale.setScalar(scale);
      var corrected=new THREE.Box3().setFromObject(gltf.scene);gltf.scene.position.y-=corrected.min.y;
      gltf.scene.traverse(function(o){if(o.isMesh){o.castShadow=true;o.receiveShadow=true;o.frustumCulled=true}});
      pugRoot.add(gltf.scene);glb.active=true;glb.model=gltf.scene;glb.mixer=mixer;glb.actions=actions;proceduralPug.visible=false;
    },undefined,function(){});
  }catch(error){}
}
...
function animatePug(dt){
  ...
  P.body.position.copy(pugBase.bodyPos);P.chest.position.copy(pugBase.chestPos);P.rear.position.copy(pugBase.rearPos);P.belly.position.copy(pugBase.bellyPos);
  P.shoulderL.position.copy(pugBase.shoulderL);P.shoulderR.position.copy(pugBase.shoulderR);P.haunchL.position.copy(pugBase.haunchL);P.haunchR.position.copy(pugBase.haunchR);
  P.body.scale.copy(pugBase.bodyScale);P.chest.scale.copy(pugBase.chestScale);P.rear.scale.copy(pugBase.rearScale);P.belly.scale.copy(pugBase.bellyScale);P.nose.scale.copy(pugBase.noseScale);
  P.body.rotation.set(0,0,0);P.chest.rotation.set(0,0,0);P.rear.rotation.set(0,0,0);P.shoulderL.rotation.set(0,0,0);P.shoulderR.rotation.set(0,0,0);
  P.head.position.set(0,.625,.37);P.head.rotation.set(0,0,0);P.jaw.rotation.x=0;P.jaw.position.set(0,-.132,.175);
  [P.legFL,P.legFR,P.legRL,P.legRR].forEach(function(l){l.rotation.set(0,0,0);l.scale.set(1,1,1)});
  P.tongue.visible=false;P.tongue.position.set(0,-.154,.205);P.eyes.forEach(function(e,index){e.scale.copy(pugBase.eyeScales[index])});P.brows[0].rotation.z=.26;P.brows[1].rotation.z=-.18;
  P.ears[0].rotation.set(.52,-.06,-.5);P.ears[1].rotation.set(.52,.06,.5);P.tailGroup.rotation.set(0,0,0);
  pugShadow.scale.set(1,1,1);pugShadow.material.opacity=.46;
  var talking=voiceState.speaking&&nowMs()<voiceState.until;
  if(talking){
    var mouth=Math.max(0,Math.sin(nowMs()*.019));P.jaw.rotation.x=-mouth*.31;P.jaw.position.y=-.132-mouth*.009;P.head.rotation.y=Math.sin(t*1.35)*.075;P.head.rotation.x=-.035;P.brows[0].rotation.z=.34+mouth*.05;P.brows[1].rotation.z=-.24-mouth*.04;
  }
  ...
  var run=clamp((pug.move-.62)/.38,0,1),moving=st==='walk'?clamp(pug.move,.16,1):0;
  pug.settle=lerp(pug.settle,moving,clamp(dt*6,0,1));
  if(st==='walk'||st==='idle'){
    var amp=lerp(.48,.67,run)*moving,step=Math.sin(phase),step2=Math.sin(phase+Math.PI);
    P.legFL.rotation.x=step*amp;P.legRR.rotation.x=step*amp;P.legFR.rotation.x=step2*amp;P.legRL.rotation.x=step2*amp;
    proceduralPug.position.y=Math.abs(step)*lerp(.018,.036,run)*moving;
    proceduralPug.rotation.z=step*lerp(.055,.105,run)*moving;
    P.rear.position.x=step*.032*moving;P.rear.rotation.z=step*.065*moving;
    P.body.rotation.z=step*.035*moving;P.chest.rotation.z=-step*.026*moving;
    P.haunchL.position.y=pugBase.haunchL.y+Math.max(0,step)*.012*moving;P.haunchR.position.y=pugBase.haunchR.y+Math.max(0,-step)*.012*moving;
    P.ears[0].rotation.x=.52+Math.abs(step)*lerp(.025,.18,run)*moving;P.ears[1].rotation.x=.52+Math.abs(step2)*lerp(.025,.18,run)*moving;
    pug.lookT-=dt;if(pug.lookT<=0){pug.lookTarget=rand(-.5,.5);pug.lookT=rand(2.6,5.6)}
    pug.look=lerp(pug.look,pug.lookTarget,dt*2.4);if(!talking){P.head.rotation.y=pug.look;P.head.rotation.z=-step*.026*moving+Math.sin(t*.7)*.015*(1-moving)}
    pug.lickT-=dt;if(pug.lickT<=0){pug.lick=.68;pug.lickT=rand(8,15)}
    if(pug.lick>0||run>.82){pug.lick-=dt;P.tongue.visible=true;P.tongue.position.y=-.154+Math.sin(t*11)*.008;P.jaw.rotation.x=Math.min(P.jaw.rotation.x,-.12-run*.08)}
    var breathe=1+Math.sin(t*(run?3.2:1.75))*.014;
    P.body.scale.y=pugBase.bodyScale.y*breathe;P.chest.scale.y=pugBase.chestScale.y*(1+(breathe-1)*1.2);P.belly.scale.y=pugBase.bellyScale.y*(1+(breathe-1)*1.35);
    if(st==='idle'&&!talking){proceduralPug.position.x=Math.sin(t*.65)*.006;P.head.rotation.x=Math.sin(t*.82)*.018;P.head.rotation.z+=Math.sin(t*.43)*.025}
  }else if(st==='sit'||st==='beg'){
    proceduralPug.rotation.x=-.31;proceduralPug.position.y=.045;P.rear.position.y=.27;P.belly.position.y=.16;
    P.legRL.scale.y=.42;P.legRR.scale.y=.42;P.legRL.rotation.x=-.82;P.legRR.rotation.x=-.82;P.legFL.scale.y=1.35;P.legFR.scale.y=1.35;
    P.head.rotation.x=st==='beg'?-.07:.22;P.head.position.y=.635;
    pugShadow.scale.set(1.08,.92,1);pugShadow.material.opacity=.5;
    if(st==='beg'){P.legFR.rotation.x=-1.18+Math.sin(t*6)*.07;P.head.rotation.z=Math.sin(t*1.4)*.07;P.eyes.forEach(function(e,index){e.scale.copy(pugBase.eyeScales[index]).multiplyScalar(1.075)})}
  }else if(st==='sniff'){
    proceduralPug.position.y=-.025;P.head.position.set(0,.5,.44);P.head.rotation.x=.56;var nosePulse=1+Math.sin(t*20)*.12;P.nose.scale.copy(pugBase.noseScale).multiplyScalar(nosePulse);proceduralPug.rotation.z=Math.sin(t*5)*.018;P.shoulderL.rotation.x=.08;P.shoulderR.rotation.x=.08;
  }else if(st==='dig'){
    ...
    P.legFL.rotation.x=Math.sin(t*10)*.54;P.legRR.rotation.x=Math.sin(t*10)*.54;P.legFR.rotation.x=Math.sin(t*10+Math.PI)*.54;P.legRL.rotation.x=Math.sin(t*10+Math.PI)*.54;proceduralPug.position.y=Math.abs(Math.sin(t*10))*.022;
  }else if(st==='scratch'){
    proceduralPug.rotation.x=.11;P.head.rotation.x=-.25;P.legFL.rotation.x=-.43+Math.sin(t*14)*.46;P.legFR.rotation.x=-.28+Math.sin(t*14+1.7)*.35;
  }else if(st==='lie'||st==='sleep'){
    proceduralPug.position.y=-.20;P.body.rotation.x=.04;P.legFL.scale.y=.28;P.legFR.scale.y=.28;P.legRL.scale.y=.28;P.legRR.scale.y=.28;P.legFL.rotation.x=-.9;P.legFR.rotation.x=-.9;P.legRL.rotation.x=.7;P.legRR.rotation.x=.7;P.head.position.set(0,.47,.45);P.head.rotation.x=.25;
    var breath=1+Math.sin(t*(st==='sleep'?1.03:1.65))*(st==='sleep'?.03:.016);P.body.scale.y=pugBase.bodyScale.y*breath;P.belly.scale.y=pugBase.bellyScale.y*(1+(breath-1)*1.5);P.chest.scale.y=pugBase.chestScale.y*(1+(breath-1)*1.25);
    pugShadow.scale.set(1.16,.84,1);pugShadow.material.opacity=.52;
    if(st==='sleep'){P.eyes.forEach(function(e,index){e.scale.y=pugBase.eyeScales[index].y*.055});if(pug.jolt>0){pug.jolt-=dt*2;P.head.rotation.x+=Math.sin(t*24)*.055*pug.jolt;P.legRL.rotation.x+=Math.sin(t*30)*.08*pug.jolt}}
  }else if(st==='hop'||st==='jump'){
    P.legFL.rotation.x=-.68;P.legFR.rotation.x=-.68;P.legRL.rotation.x=.68;P.legRR.rotation.x=.68;P.head.rotation.x=-.16;pugShadow.scale.set(.8,.72,1);pugShadow.material.opacity=.32;
  }
  P.tailGroup.rotation.y=Math.sin(t*(3.5+pug.move*6))*(.055+pug.move*.11);P.tailGroup.rotation.x=Math.sin(t*2.1)*.025;
}
...
cylinder(.29,.31,.12,M.robot,0,.08,0,vacuumRoot,20);cylinder(.2,.2,.04,M.metal,0,.16,0,vacuumRoot,18);var vacuumLed=sphere(.032,.032,.032,new THREE.MeshBasicMaterial({color:0x79ff9b}),0,.14,.2,vacuumRoot,8);box(.46,.15,.29,M.robot,-1.9,.075,-6.5);
...
smellPoints.forEach(function(s,index){
  s.group=new THREE.Group();s.group.position.set(s.x,.03,s.z);scene.add(s.group);s.wisps=[];s.phase=index*.8;
  var glowMat=new THREE.MeshBasicMaterial({color:0xb9e786,transparent:true,opacity:.16,depthWrite:false});
  var base=mesh(new THREE.CircleGeometry(.42,24),glowMat,0,0,0,s.group);base.rotation.x=-Math.PI/2;
  var ring=mesh(new THREE.RingGeometry(.33,.39,24),new THREE.MeshBasicMaterial({color:0xe4ffb4,transparent:true,opacity:.38,depthWrite:false}),0,.008,0,s.group);ring.rotation.x=-Math.PI/2;s.ring=ring;
  for(var i=0;i<5;i++){
    var puff=sphere(.045+i*.005,.07+i*.006,.045+i*.005,new THREE.MeshBasicMaterial({color:i%2?0xd6ffaf:0xaedc83,transparent:true,opacity:.34,depthWrite:false}),rand(-.22,.22),.08+i*.095,rand(-.22,.22),s.group,9);puff.castShadow=false;s.wisps.push(puff);
  }
...
/* Camera */
var cam={yaw:-2.4,pitch:.32,distance:4.35,focus:new THREE.Vector3(.6,.47,-1.4),look:new THREE.Vector3(.6,.47,-1.4),targetPos:new THREE.Vector3(),ray:new THREE.Vector3()};
...
function updateCamera(dt){
  var k=1-Math.exp(-dt*(settings.calm?2.15:5.4));
  var talkZoom=voiceState.speaking&&pug.move<.18&&!Game.sleeping?.24:0;
  cam.distance=lerp(cam.distance,desiredDistance()-talkZoom,k);cam.pitch=lerp(cam.pitch,Game.sleeping?.48:.32,k);
  if(Game.mode==='finale'&&!settings.calm)cam.yaw+=dt*.038;
  var fx=Game.sleeping?bedCX:pug.pos.x,fy=Game.sleeping?.85:.46,fz=Game.sleeping?bedCZ:pug.pos.z;
  cam.focus.set(fx,fy,fz);
  var h=cam.distance*Math.cos(cam.pitch),v=cam.distance*Math.sin(cam.pitch)+.23;
  cam.targetPos.set(fx-Math.sin(cam.yaw)*h,fy+v,fz-Math.cos(cam.yaw)*h);
  if(!Game.sleeping){
    cam.ray.copy(cam.targetPos).sub(cam.focus);var max=1;
    for(var i=1;i<=18;i++){var t=i/18,px=fx+cam.ray.x*t,pz=fz+cam.ray.z*t,py=fy+cam.ray.y*t;if(blocked(px,pz,.09)||py<.34){max=Math.max(.16,(i-1)/18);break}}
    if(max<1)cam.targetPos.copy(cam.focus).addScaledVector(cam.ray,max*.94);
  }
  if(cam.targetPos.y<.4)cam.targetPos.y=.4;
  camera.position.lerp(cam.targetPos,k);cam.look.lerp(cam.focus,1-Math.exp(-dt*(settings.calm?3.1:7.6)));camera.lookAt(cam.look);
}
...
function updatePlaying(dt){
  ...
  pugRoot.position.set(pug.pos.x,pug.groundY+pug.visualY,pug.pos.z);pugRoot.rotation.y=pug.yaw;pugShadow.position.y=.012-pug.visualY;
...
function resetGame(){
  ...
  humanRoot.position.set(HUMAN_KITCHEN.x,0,HUMAN_KITCHEN.z);humanRoot.rotation.y=HUMAN_KITCHEN.yaw;human.target=null;human.lookT=2;human.yawTarget=HUMAN_KITCHEN.yaw;human.bob=0;H.eyeSprite.visible=false;
  ...
  pug.pos.set(.6,0,-1.4);pug.vel.set(0,0,0);pug.yaw=-Math.PI/2;pug.groundY=0;pug.visualY=0;pug.onBed=false;pug.forcedYaw=null;pug.move=0;pug.jolt=0;pug.digClock=0;pug.settle=0;proceduralPug.visible=!glb.active;setPugState('lie');pugRoot.position.set(.6,0,-1.4);pugRoot.rotation.y=pug.yaw;
  cam.yaw=-2.4;cam.pitch=.32;cam.distance=4.35;cam.focus.set(.6,.47,-1.4);cam.look.copy(cam.focus);camera.position.set(3.7,2.0,2.1);
...
function applyRendererSize(){var dpr=Math.min(devicePixelRatio||1,1.85),scale=settings.quality==='low'?.62:settings.quality==='medium'?.8:.95;renderer.setPixelRatio(dpr*scale);renderer.setSize(innerWidth,innerHeight,false)}
...
if(DEBUG){
  ...
  setInterval(function(){var el=$('debug-info');if(el)el.textContent='mode '+Game.mode+' / q'+Game.quest+' / fps target / '+(TelegramApp.active?'tg':'browser')},800);
}
